...
vector<pii> rangeMaxXup;
vector<pii> rangeMaxXdown;
vector<pii> rangeMaxYleft;
vector<pii> rangeMaxYright;
vi adj[MAX_N];
int N,r,d;
int size;
int start;
...
pii query(int nodeLeft, int nodeRight, int left, int right, int node, vector<pii>& v) {
...
void update(int nodeLeft, int nodeRight, int node, int pos, int val, int id, vector<pii>& v) {
...
void update(int pos, int val, int id, vector<pii>& v){
	update(0,size-1,1,pos,val,id,v);
}

pii query(int left, int right, vector<pii>& v) {
...
int main() {
	...
	inp2(N,r);
	size=10010002;
	rangeMaxXup.resize(4*size);
	rangeMaxXdown.resize(4*size);
	rangeMaxYleft.resize(4*size);
	rangeMaxYright.resize(4*size);
	FOR(i,N){
		int x,y;
		inp2(x,y);
