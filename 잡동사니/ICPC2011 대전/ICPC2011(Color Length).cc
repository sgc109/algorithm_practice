...
void solve(int pos1, int pos2){
	if(pos1 == len1 && pos2 == len2) {
		int ret=0;
		FOR(i,26) {
			if(maxPos[i]!=-INF || minPos[i]!=INF) ret+=maxPos[i]-minPos[i];
			else ret+=0;
		}
		ans=min(ans,ret);
		return;
	...
	if(pos2 != len2){
		int minBack = minPos[s2[pos2]-'A'];
		int maxBack = maxPos[s2[pos2]-'A'];
		minPos[s2[pos2]-'A'] = min(minPos[s2[pos2]-'A'], pos1+pos2);
		maxPos[s2[pos2]-'A'] = max(maxPos[s2[pos2]-'A'], pos1+pos2);
		solve(pos1,pos2+1);
		minPos[s2[pos2]-'A'] = minBack;
		maxPos[s2[pos2]-'A'] = maxBack;
	}
	if(pos1 != len1){
		int minBack = minPos[s1[pos1]-'A'];
		int maxBack = maxPos[s1[pos1]-'A'];
		minPos[s1[pos1]-'A'] = min(minPos[s1[pos1]-'A'], pos1+pos2);
		maxPos[s1[pos1]-'A'] = max(maxPos[s1[pos1]-'A'], pos1+pos2);
		solve(pos1+1,pos2);
		minPos[s1[pos1]-'A'] = minBack;
		maxPos[s1[pos1]-'A'] = maxBack;
	}
}
int main() {
	int T;
