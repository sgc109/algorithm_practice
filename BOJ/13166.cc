...
bool dfs(int here, int limit){
	for(auto p : G[here]){
		int there = p.first;
		int lower = p.second;
		if(lower > limit) continue;
		if(matchB[there]==-1 || (level[matchB[there]]==level[here]+1 && dfs(matchB[there],limit))){
			matchB[there] = here;
			matchA[here] = there;
			return true;
		}
	...
	while(1){
		...
		while(!q.empty()){
			...
			for(auto p : G[here]){
				int there = p.first;
				int lower = p.second;
				if(matchB[there]==-1 || level[matchB[there]]!=-1 || lower > limit) continue;
