...
public:
    int countScore(vector<int> points, vector<int> skills, int luck) {
        ...
        vector<int> lucks(3,0);
        vector<int> gob({2,4,8});
        do{
            for(lucks[0] = 0; lucks[0] <= luck; lucks[0]++){
                for(lucks[1] = 0; lucks[1] <= luck-lucks[0]; lucks[1]++){
                    lucks[2] = luck - lucks[0] - lucks[1];
                    int score = 0;
                    int tsum = 0;
                    for(int i = 0; i < 3; i++){
                        int turn = perm[i];
                        int t = max(1, skills[turn] - lucks[turn]);
                        if(tsum + t > 75) break;
                        tsum += t;
                        score += max(0, points[turn] - gob[turn] * t);
                    }
                    ans = max(ans, score);        
                }
            }
