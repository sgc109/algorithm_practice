...
class YetAnotherIncredibleMachine {
    ...
    int countWays(vector<int> platformMount, vector<int> platformLength, vector<int> balls) {
        ...
        sort(all(balls));
        ll ans = 1;
        FOR(i,sz(platformMount)){
            int pos = lower_bound(all(balls),platformMount[i]) - balls.begin();
            int l = pos - 1;
            int r = pos;
            int left = max(platformMount[i] - platformLength[i], balls[l] + 1);
            int right = min(platformMount[i], balls[r] - platformLength[i]- 1);
            if(left > right) ans=0;
