...
public:
    ...
    int N,M;
    int visited[53];
    bool dfs(int here, int dad, int pos){
        // printf("here:%d, dad:%d, pos:%d\n",here,dad,pos);
        visited[here] = 1;
        FOR(there, N){
            if(!G[pos][here][there] || there == dad) continue;
            if(visited[there]) return false;
            if(!dfs(there, here, pos)) return false;
        }
        return true;
    }
    ...
    ll go(int pos, int u, int u0){
        ...
        ll ret = 0;
        if(pos==M){
            int v = parent[(pos+M-1)%M][u];
            int v0 = parent[0][u0];
            if(check(0, u0, v0, u, v)) return 1;
            return 0;
        }
        for(int i = 0; i < N; i++){
            int p = parent[pos][i];
            ...
            //     if(G[pos][i][p]!=1){
            //         printf("val:%d\n",G[pos][i][p]);
            //         printf("pos:%d, i:%d, p:%d\n",pos,i,p);
            //     }
            //     assert(G[pos][i][p]==1);
            // }
            int v;
            if(pos > 0) v = parent[(pos+M-1)%M][u];
            if(pos>0 && !check(pos, i, p, u, v)) continue;
