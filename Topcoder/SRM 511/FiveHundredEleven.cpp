...
public:
    ...
    int isWinning(int mem, int played){
        if(mem==511) return 1;
        if(played==card.size()) return 0;
        int& cache = dp[mem][played];
        if(cache!=-1) return cache;
        int cnt=0;
        int ret=0;
        FOR(i,card.size()){
            if((mem|card[i])==mem) cnt++;
            else ret|=!isWinning(mem|card[i],played+1);
        }
        if(played < cnt) ret|=!isWinning(mem,played+1);
        return cache=ret;
    }
    string theWinner(vector<int> cards) {
        memset(dp,-1,sizeof(dp));
        card =  cards;
        if(isWinning(0,0)) return "Fox Ciel";
        return "Toastman";
    }
};
